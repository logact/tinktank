...
### 主从复制了解吗？
![[Pasted image 20221014172142.png]]
...
![[Pasted image 20221014172208.png]]
一主多从结构（又称为星形拓扑结构）使得应用端可以利用多个从节点实现读写分离（见图6-5）。对于读占比较大的场景，可以把读命令发送到从节点来分担主节点压力
![[Pasted image 20221014172228.png]]
树状主从结构（又称为树状拓扑结构）使得从节点不但可以复制主节点数据，同时可以作为其他从节点的主节点继续向下层复制。通过引入复制中间层，可以有效降低主节点负载和需要传送给从节点的数据量。
![[Pasted image 20221014172244.png]]
### Redis的主从复制原理了解吗？
Redis主从复制的工作流程大概可以分为如下几步
![[Pasted image 20221014172301.png]]
...
![[Pasted image 20221014172347.png]]
**全量复制** 一般用于初次复制场景，Redis早期支持的复制功能只有全量复制，它会把主节点全部数据一次性发送给从节点，当数据量较大时，会对主从节点和网络造成很大的开销。
全量复制的完整运行流程如下：
![[Pasted image 20221014172402.png]]
...
![[Pasted image 20221014173106.png]]
...
![[Pasted image 20221014173307.png]]
![[Pasted image 20221014173317.png]]
...
![[Pasted image 20221014173415.png]]
...
![[Pasted image 20221014173528.png]]
...
![[Pasted image 20221014173728.png]]
...
选出新的主节点，大概分为这么几步:
![[Pasted image 20221014173803.png]]
...
![[Pasted image 20221014174555.png]]
...
![[Pasted image 20221014174946.png]]
...
![[Pasted image 20221014174957.png]]
...
![[Pasted image 20221014175011.png]]
...
这个方案 一致性哈希分区的基础上，引入了 **虚拟节点** 的概念。Redis 集群使用的便是该方案，其中的虚拟节点称为 **槽（slot）**。槽是介于数据和实际节点之间的虚拟概念，每个实际节点包含一定数量的槽，每个槽包含哈希值在一定范围内的数据。
![[Pasted image 20221014175034.png]]
...
![[Pasted image 20221014175142.png]]
...
![[Pasted image 20221014175323.png]]
...
![[Pasted image 20221014175544.png]]
...
![[Pasted image 20221014175605.png]]
...
![[Pasted image 20221014180247.png]]
缓存空值有两大问题：
...
![[Pasted image 20221014180315.png]]
![[Pasted image 20221014180413.png]]

#####   缓存雪崩

某⼀时刻发⽣⼤规模的缓存失效的情况，例如缓存服务宕机、大量key在同一时间过期，这样的后果就是⼤量的请求进来直接打到DB上，可能导致整个系统的崩溃，称为雪崩。
![[Pasted image 20221014180512.png]]
...
![[Pasted image 20221014180629.png]]
...
![[Pasted image 20221014180819.png]]
...
![[Pasted image 20221014180936.png]]
...
![[Pasted image 20221014181128.png]]
...
![[Pasted image 20221014183444.png]]
...
![[Pasted image 20221014183640.png]]
...
![[Pasted image 20221014183700.png]]
...
Redis主要有2种过期数据回收策略：
![[Pasted image 20221014185018.png]]
**惰性删除**
...
![[Pasted image 20221014185039.png]]
...
![[Pasted image 20221014185355.png]]
-   **API或数据结构使用不合理**
...
![[Pasted image 20221014185554.png]]
-   **删除大key**
...
6. 为了 Master 的稳定性，主从复制不要用图状结构，用单向链表结构更稳定，即主从关为：Master –Slave2 –Slave3…，这样的结构也方便解决单点故障问题，实现 Slave 对 Master 的替换，也即，如果 Master 挂了，可以立马启用 Slave1 做 Master，其他不变。
## Redis应用
### 使用Redis 如何实现异步队列？
...
![[Pasted image 20221014205809.png]]
...
![[Pasted image 20221014210353.png]]
...
![[Pasted image 20221014210530.png]]
**Pipelining的优势**
...
![[Pasted image 20221014210607.png]]
```
...
![[Pasted image 20221014210716.png]]
```
> setnx lock:fighter true
OK
...
![[Pasted image 20221014210736.png]]
```
...
![[Pasted image 20221014211343.png]]
我们常用的数据类型和编码对应的映射关系：
![[Pasted image 20221014211356.png]]
...
![[Pasted image 20221014211411.png]]
...
![[Pasted image 20221014211428.png]]
...
![[Pasted image 20221014211441.png]]
...
![[Pasted image 20221014211459.png]]
5. **整数集合intset**：⽤于保存整数值的集合抽象数据结构，不会出现重复元素，底层实现为数。
![[Pasted image 20221014211524.png]]
6.  **压缩列表ziplist**：压缩列表是为节约内存⽽开发的顺序性数据结构，它可以包含任意多个节点，每个节点可以保存⼀个字节数组或者整数值。
![[Pasted image 20221014211547.png]]
...
![[Pasted image 20221014211612.png]]
**C语言的字符串可能有什么问题？**
...
**Redis如何解决？优势？**
![[Pasted image 20221014211629.png]]
简单来说一下 Redis 如何解决的：
...
![[Pasted image 20221014211705.png]]
> **字典是怎么扩容的？**
...
![[Pasted image 20221014211725.png]]
> **为什么使用跳跃表?**
...
![[Pasted image 20221014211743.png]]
...
![[Pasted image 20221014211752.png]]
...
![[Pasted image 20221014211818.png]]
压缩列表由这么几部分组成：
...
-   **entryX**:列表节点
-   **zlend**:用于标记压缩列表的末端
![[Pasted image 20221014211828.png]]
### 快速列表 quicklist 了解吗？
...
![[Pasted image 20221014211842.png]]
## 其他问题
