# 参考
[面渣逆袭（MySQL面试题八股文）必看 👍 | Java 程序员进阶之路 (tobebetterjavaer.com)](https://tobebetterjavaer.com/sidebar/sanfene/mysql.html)
# 问题
## 基础
### 说一说数据库三大范式
1. 列不可分
2. 非主键列完全依赖于主键
3. 非主键只依赖于主键，而不依赖于其它非主键

### varchar 和char 的区别
**char**
1. char 是固定长度
2. char的最大长度为255与编码无关
3. 存取效率更加高由于是固定长度
**varchar**
1. varchar是可变长度的字符串
2. varchar的最大长度是65532
### blob和text有什么区别？
1. blob用于存储二进制，而text用于存储大文本
### DATETIME和TIMESTAMP的异同
**相同点** 
1. 两个数据类型存储时间的表现格式一致。均为YYYY-MM-DD HH:MM:SS
2. 两个格式都可以存储到微秒的小数秒（小数后6位小数）
**区别**
...
![[Pasted image 20221014105634.png]]
...
### 说一说mysql的基础架构
![[Pasted image 20221014110354.png]]
...
-   完成查询优化后，按照生成的执行计划`调用数据库引擎接口`，返回执行结果。
 ...
 ### MySQL 有哪些常见存储引擎
 ![[Pasted image 20221014110758.png]]
...
![[Pasted image 20221014111038.png]]
...
- binlog记录了对MySQL数据库执行更改的所有操作，但是不包括SELECT和SHOW这类操作，因为这类操作对数据本身并没有修改。然后，若操作本身并没有导致数据库发生变化，那么该操作也会写入[二进制](https://so.csdn.net/so/search?q=%E4%BA%8C%E8%BF%9B%E5%88%B6&spm=1001.2101.3001.7020)日志。例如：
...
![[Pasted image 20221014112448.png]]
...
![[Pasted image 20221014112748.png]]
...
![[Pasted image 20221014112742.png]]
...
## SQL优化
### 有哪些方式优化慢 SQL
![[Pasted image 20221014141130.png]]
#### 分页优化
...
![[Pasted image 20221014142135.png]]
![[Pasted image 20221014142139.png]]
...
### 能简单说一下索引的分类吗？
![[Pasted image 20221014142850.png]]
例如从基本使用使用的角度来讲：
...
![[Pasted image 20221014142954.png]]
### 创建索引有哪些注意点？
...
![[Pasted image 20221014144636.png]]
在这张图里，有两个重点：
...
### 那一棵 B+树能存储多少条数据呢
![[Pasted image 20221014150757.png]]
...
![[Pasted image 20221014162047.png]]
-   **Gap Lock 间隙锁**

间隙锁(Gap Locks) 的间隙指的是两个记录之间逻辑上尚未填入数据的部分,是一个**左开右开空间**。
**![[Pasted image 20221014162114.png]]
...
![[Pasted image 20221014162136.png]]
...
![[Pasted image 20221014162150.png]]
### MySQL 的乐观锁和悲观锁了解吗？
...
![[Pasted image 20221014162810.png]]
### 事务的各个隔离级别都是如何实现的？
**读未提交**
...
![[Pasted image 20221014162932.png]]
...
![[Pasted image 20221014163615.png]]
读写分离的基本实现是:
...
其基本架构是：
![[Pasted image 20221014163920.png]]
### 主从复制原理了解吗？
...
-   slave 记录自己的 binglog
![[Pasted image 20221014163942.png]]
### 主从同步延迟怎么处理？
...
![[Pasted image 20221014164219.png]]
...
![[Pasted image 20221014164229.png]]
...
![[Pasted image 20221014164242.png]]
...
![[Pasted image 20221014165842.png]]
-   **第二阶段：在线双写，查询走新库**

1.  完成了历史数据的同步和校验
2.  把对数据的读切换到新库

![[Pasted image 20221014165854.png]]
-   **旧库下线**

1.  旧库不再写入新的数据
2.  经过一段时间，确定旧库没有请求之后，就可以下线老库
![[Pasted image 20221014170147.png]]
### 常用的分库分表中间件有哪些？
